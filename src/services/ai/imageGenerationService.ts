...
export class ImageGenerationService {
  ...
  static async generateStoryImages(options: ImageGenerationOptions): Promise<ImageGenerationResult> {
    const { storyId, chapterId = 1, scenes, imageStyle } = options;

    const imageStyleId = imageStyle || DEFAULT_IMAGE_STYLE_ID;
    const providerConfig = getImageProviderConfig(imageStyleId);
...
Genera una imagen de la SEGUNDA ESCENA más importante del cuento, donde el PERSONAJE PRINCIPAL debe ser prominente en la escena. Debe representar otro momento crucial diferente al anterior, mostrando al protagonista en una situación distinta pero manteniendo continuidad visual y las características del personaje establecidas. Estilo acuarela tradicional de cuento infantil.`
  ...
  private static async generateSingleImage(
    ...
    try {
      ...
      const resolvedStoragePath = metadata.storagePath ?? null;
      const resolvedStorageBucket = metadata.storageBucket ?? null;
      const resolvedPublicUrl = edgeResponse.publicUrl ?? null;
      ...
      if (resolvedPublicUrl) {
        if (!resolvedStoragePath) {
          throw new Error('La Edge devolvió URL pública sin storagePath; ejecuta el backfill o revisa generate-image');
        }
      ...
      if (!uploadResult.publicUrl || !uploadResult.storagePath) {
        throw new Error('La subida de la ilustración no devolvió URL pública o storagePath');
      }

      const mergedMetadata: GeneratedImageMetadata = {
        ...metadata,
        storagePath: uploadResult.storagePath ?? metadata.storagePath ?? null,
        storageBucket: metadata.storageBucket ?? 'images-stories',
      ...
      try {
        const { data, error } = await supabase.functions.invoke<EdgeFunctionResponse & { success?: boolean; error?: string }>('generate-image', {
          body: {
            ...
            styleId: params.providerConfig.styleId,
            ...
            providerDefault: params.providerConfig.defaultProvider,
            providerFallback: params.providerConfig.fallbackProvider,
          },
  ...
  private static composeGeneratedMetadata(
    ...
    return {
      ...
      defaultProvider: metadata?.defaultProvider ?? undefined,
      fallbackProvider: metadata?.fallbackProvider ?? undefined,
      ...
      finalResolutionPx: metadata?.finalResolutionPx ?? undefined,
      finalLayout: metadata?.finalLayout ?? undefined,
      canvasLabel: metadata?.canvasLabel ?? undefined,
      ...
      storageBucket: metadata?.storageBucket ?? response.storageBucket ?? undefined,
