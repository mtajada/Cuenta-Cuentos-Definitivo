...
interface ImageGenerationOptions {
  ...
  scenes: StoryScenes; // NUEVO: recibe scenes de la BD
}
...
export class ImageGenerationService {
  ...
  static async generateStoryImages(options: ImageGenerationOptions): Promise<ImageGenerationResult> {
    ...
    try {
      console.log('[ImageGeneration] Starting image generation with AI-provided prompts for story:', storyId);
      
      // Use prompts directly from database (generated by story AI)
      ...
      const worker = async () => {
        while (queue.length > 0) {
          ...
          try {
            ...
            if (!prompt) {
              throw new Error(`Missing prompt for ${imageType}`);
            }
            console.log(`[ImageGeneration] Generating ${imageType} with AI-provided prompt...`);
            ...
            if (result.success && result.image) {
              ...
              console.warn(`[ImageGeneration] ⚠️ Failed to generate ${imageType}`);
            }
          } catch (error) {
      ...
      console.log(`[ImageGeneration] Completed: ${successfulImages.length}/${imageTypes.length} images successful`);
...
Genera una imagen de la SEGUNDA ESCENA más importante del cuento, donde el PERSONAJE PRINCIPAL debe ser prominente en la escena. Debe representar otro momento crucial diferente al anterior, mostrando al protagonista en una situación distinta pero manteniendo continuidad visual y las características del personaje establecidas. Estilo acuarela tradicional de cuento infantil.`
  ...
  private static async generateSingleImage(
    ...
    try {
      ...
      // Prefer server-side upload to avoid large payloads and double roundtrips
      const { publicUrl } = await this.callGenerateImageEdge({
        prompt,
        storyId,
      ...
      // Fallback: if server-side upload not available, do legacy path
      const imageBase64 = await this.callGenerateImageEdge({ prompt }).then(r => r.imageBase64);
      if (!imageBase64) throw new Error('No image data returned from image service');
      ...
      console.log(`[ImageGeneration] Successfully generated and uploaded ${imageType} (fallback)`);
      return {
        success: true,
        image: {
          type: imageType,
   ...
   */
  private static async callGenerateImageEdge(params: { prompt: string; storyId?: string; chapterId?: string; imageType?: string }): Promise<{ imageBase64?: string | null; publicUrl?: string | null }> {
    try {
      ...
      try {
        ...
        return { imageBase64: data?.imageBase64, publicUrl: data?.publicUrl };
      } catch (invokeError) {
        clearTimeout(timeout);
