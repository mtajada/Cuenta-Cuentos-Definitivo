...
const GEMINI_COMPATIBLE_ENDPOINT = Deno.env.get("GEMINI_COMPATIBLE_ENDPOINT") ||
...
if (!GEMINI_COMPATIBLE_ENDPOINT) {
  throw new Error("GEMINI_COMPATIBLE_ENDPOINT environment variable not set");
}
...
console.log(
  `story-continuation ${functionVersion}: Using model ${TEXT_MODEL_GENERATE} via ${openai.baseURL}`,
);
...
function isValidOptionsResponse(
  data: any,
): data is AiContinuationOptionsResponse {
  return data &&
    Array.isArray(data.options) &&
    data.options.every((opt: any) =>
      typeof opt.summary === "string" && opt.summary.trim() !== ""
    );
}

function isValidContinuationResponse(
  data: any,
): data is AiContinuationResponse {
  return data &&
...
async function generateContinuationOptions(
  ...
  console.log(
    `[${functionVersion}] generateContinuationOptions. reqId=${
      requestId || "unknown"
    } storyId=${story?.id}`,
  );
  ...
  if (!story || !story.id || !story.title || !story.content || !story.options) {
    throw new Error(
      "Datos de historia inválidos/incompletos para generar opciones.",
    );
  }
  ...
  const prompt = createContinuationOptionsPrompt(
    story,
    chapters,
    language,
    childAge,
  ...
  console.log(
    `[${functionVersion}] Prompt para generación de opciones (lang: ${language}):\n---\n${
  ...
  try {
    ...
    try {
      ...
      console.error(
        `[${functionVersion}] Upstream AI error (options). reqId=${
          requestId || "unknown"
        } userId=${userId || "unknown"} status=${
          err?.status ?? "unknown"
        } request_id=${err?.request_id ?? "unknown"}`,
      );
      console.error(
        `[${functionVersion}] Upstream AI error headers (selected, options). reqId=${
          requestId || "unknown"
        }`,
        {
      ...
      console.error(
        `[${functionVersion}] Upstream AI error message (options). reqId=${
          requestId || "unknown"
        }`,
      ...
      console.log(
        `[${functionVersion}] AI call finished (options). reqId=${
          requestId || "unknown"
    ...
    console.log(
      `[${functionVersion}] Raw AI JSON for options (first 200 chars): ${
        aiResponseContent?.substring(0, 200) || "(No content received)"
      }... Finish Reason: ${finishReason}`,
    );

    if (finishReason === "length") {
      console.warn(
        `[${functionVersion}] AI option generation may have been truncated.`,
      );
    }
    ...
    try {
      ...
      console.log(
        `[${functionVersion}] Options JSON parsed successfully on first try`,
      );
    } catch (parseError) {
      const errorMsg = parseError instanceof Error
        ...
        : "Unknown error";
      console.error(
        `[${functionVersion}] Failed to parse options JSON${
          userId ? ` (User: ${userId})` : ""
        }. Error: ${errorMsg}. Trying aggressive cleaning...`,
      );
      console.log(
        `[${functionVersion}] Problematic JSON (first 500 chars): ${
      ...
      try {
        ...
        console.log(
          `[${functionVersion}] Cleaned JSON (first 400 chars): ${
            cleanedContent.substring(0, 400)
          }`,
        );
        parsedResponse = JSON.parse(cleanedContent);
        console.log(
          `[${functionVersion}] Options JSON parsed successfully with aggressive cleaning`,
        );
      } catch (fallbackError) {
        const fallbackMsg = fallbackError instanceof Error
          ...
          : "Unknown error";
        console.error(
          `[${functionVersion}] Aggressive cleaning parsing also failed${
            userId ? ` (User: ${userId})` : ""
          }: ${fallbackMsg}`,
        );
        console.error(
          `[${functionVersion}] Raw response that failed${
            userId ? ` (User: ${userId})` : ""
          }: ${aiResponseContent}`,
        );
        throw new Error(
          `No se pudo parsear la respuesta JSON de opciones: ${errorMsg}`,
        );
      }
    }
    ...
    if (isValidOptionsResponse(parsedResponse)) {
      console.log(
        `[${functionVersion}] Opciones JSON parseadas y validadas:`,
        parsedResponse.options,
      );
      return parsedResponse; // Return the whole object: { options: [...] }
    }
    console.error(
      `[${functionVersion}] Formato de opciones inválido después de parsear. Data:`,
      parsedResponse,
    );
    throw new Error(
      "Formato de opciones inválido después de parsear el JSON de la IA.",
    );
  } catch (e: any) {
    console.error(
      `[${functionVersion}] Error procesando la respuesta de la IA para las opciones: ${e.message}. Raw response: ${
        aiResponseContent?.substring(0, 500)
      }`,
      e,
    );
    ...
    const defaultOptions = [
      ...
      summary: `${opt.summary} (${
        language.startsWith("en") ? "default option" : "opción por defecto"
      })`,
    }));
...
function cleanExtractedText(
  ...
  const defaultText = type === "title"
    ? `Un Nuevo Capítulo`
    : "La historia continúa de forma misteriosa...";
  if (text === null || text === undefined || typeof text !== "string") { // Allow empty string from AI, will return default
    console.warn(
      `[${functionVersion}] cleanExtractedText (${type}): Input null, undefined, or not a string.`,
    );
    return defaultText;
  }
  ...
  cleaned = cleaned.replace(
    /^(Título|Title|Contenido|Content|Respuesta|Response):\s*/i,
    "",
  ).trim();
  cleaned = cleaned.replace(
    /^(Aquí tienes el (título|contenido|cuento|capítulo)|Claro, aquí está el (título|contenido|cuento|capítulo)):\s*/i,
    "",
  ).trim();
...
serve(async (req: Request) => {
  ...
  try {
    ...
    try {
      ...
      console.error(
        `[${functionVersion}] Failed to parse JSON body. reqId=${requestId} userId=${userId}. Error:`,
        error,
      );
    ...
    const requiresStoryForContext = isContinuationAction ||
    ...
    if (requiresStoryForContext) {
      if (!story || typeof story !== "object" || !story_id) {
        throw new Error(
          `Objeto 'story' (con 'id') inválido/ausente para la acción '${action}'.`,
        );
      }
      ...
      const hasCharacterData =
        ...
        story.options.character?.name;
      if (
        !story.content || !story.options || !hasCharacterData || !story.title
      ) {
        ...
        throw new Error(
          "Datos incompletos en el objeto 'story' recibido (content, options con al menos un personaje, title son necesarios).",
        );
      }
      if (!Array.isArray(chapters)) {
        throw new Error(
          `Array 'chapters' requerido (puede ser vacío) para la acción '${action}'.`,
        );
      }
    }
    if (
      action === "optionContinuation" &&
      (typeof selectedOptionSummary !== "string" ||
        !selectedOptionSummary.trim())
    ) {
      throw new Error(
        "'selectedOptionSummary' (string no vacío) requerido para 'optionContinuation'.",
      );
    }
    if (
      action === "directedContinuation" &&
      (typeof userDirection !== "string" || !userDirection.trim())
    ) {
      throw new Error(
        "'userDirection' (string no vacío) requerido para 'directedContinuation'.",
      );
    }

    const language = body.language || story?.options?.language || "Español";
    const childAge = body.childAge || story?.options?.childAge || 7;
    const specialNeed = body.specialNeed || story?.options?.specialNeed ||
      "Ninguna";
    const storyDuration = body.storyDuration || story?.options?.duration ||
      "medium";
    ...
    if (isContinuationAction) {
      ...
      if (profileError) {
        throw new Error(
          "Error al verificar el perfil de usuario para límites.",
        );
      }
    ...
    console.log(
      `[${functionVersion}] Executing action: ${action} for user ${userId}, story ${
        story_id || "N/A"
      }`,
    );
    ...
    if (action === "generateOptions") {
      ...
      if (action === "optionContinuation") {
        continuationContext.optionSummary = selectedOptionSummary;
      }
      if (action === "directedContinuation") {
        continuationContext.userDirection = userDirection;
      }
      ...
      console.log(
        `[${functionVersion}] Calling AI for continuation. reqId=${requestId} userId=${userId} promptLength=${continuationPrompt.length}`,
      );
      ...
      try {
        ...
        const upstreamHeaders = normalizeHeaderRecord(err?.headers);
        console.error(
          `[${functionVersion}] Upstream AI error (continuation). reqId=${requestId} userId=${userId} status=${
            err?.status ?? "unknown"
          } request_id=${err?.request_id ?? "unknown"}`,
        );
        console.error(
          `[${functionVersion}] Upstream AI error headers (selected, continuation). reqId=${requestId}`,
          {
            "retry-after": upstreamHeaders["retry-after"],
            "x-request-id": upstreamHeaders["x-request-id"],
            "x-ratelimit-limit-requests":
        ...
        console.error(
          `[${functionVersion}] Upstream AI error message (continuation). reqId=${requestId}`,
          err?.message ?? upstreamError,
        );
        throw upstreamError;
      } finally {
        console.log(
          `[${functionVersion}] AI call finished (continuation). reqId=${requestId} durationMs=${
            Date.now() - aiStartMs
          }`,
        );
      }
      ...
      const finishReason = chatCompletion.choices[0]?.finish_reason;
      console.log(
        `[${functionVersion}] Raw AI JSON for continuation (first 200 chars): ${
          aiResponseContent?.substring(0, 200) || "(No content received)"
        }... Finish Reason: ${finishReason}`,
      );
      ...
      if (finishReason === "content_filter") {
        console.error(
          `[${functionVersion}] AI Continuation Generation BLOCKED due to content filter.`,
        );
        throw new Error(
          `Generación de continuación bloqueada por seguridad: filtro de contenido.`,
        );
      }
      if (finishReason === "length") {
        console.warn(
          `[${functionVersion}] AI continuation generation may have been truncated.`,
        );
      }
      if (!aiResponseContent) {
        throw new Error(
          "Fallo al generar continuación: Respuesta IA vacía (sin bloqueo explícito).",
        );
      }
      ...
      try {
        ...
        if (isValidContinuationResponse(parsedResponse)) {
          ...
          console.log(
            `[${functionVersion}] Parsed AI continuation JSON successfully on first try.`,
          );
        } else {
          console.warn(
            `[${functionVersion}] AI continuation response JSON structure invalid. Data:`,
            parsedResponse,
          );
        }
      } catch (parseError) {
        const errorMsg = parseError instanceof Error
          ...
          : "Unknown error";
        console.error(
          `[${functionVersion}] Failed to parse JSON from AI continuation response${
            userId ? ` (User: ${userId})` : ""
          }. Error: ${errorMsg}. Trying aggressive cleaning...`,
        );
        console.log(
          `[${functionVersion}] Problematic continuation JSON (first 500 chars): ${
            aiResponseContent.substring(0, 500)
          }`,
        );
        ...
        try {
          ...
          console.log(
            `[${functionVersion}] Cleaned continuation JSON (first 400 chars): ${
              cleanedContent.substring(0, 400)
            }`,
          );
          ...
          if (isValidContinuationResponse(parsedResponse)) {
            ...
            parsedSuccessfully = true;
            console.log(
              `[${functionVersion}] Parsed AI continuation JSON successfully with aggressive cleaning.`,
            );
          }
        } catch (fallbackError) {
          const fallbackMsg = fallbackError instanceof Error
          ...
          try {
            console.log(
              `[${functionVersion}] Attempting manual extraction with regex${
                userId ? ` (User: ${userId})` : ""
              }...`,
            );
            ...
            if (titleMatch && contentMatch) {
              ...
              parsedSuccessfully = true;
              console.log(
                `[${functionVersion}] Extracted continuation content using robust regex. Title: "${
                  finalTitle.substring(0, 50)
                }..."`,
              );
            } else {
              console.warn(
                `[${functionVersion}] Regex extraction failed${
                  userId ? ` (User: ${userId})` : ""
                }. TitleMatch: ${!!titleMatch}, ContentMatch: ${!!contentMatch}`,
              );
              console.error(
                `[${functionVersion}] Raw response that failed all parsing attempts${
                  userId ? ` (User: ${userId})` : ""
                }: ${aiResponseContent}`,
              );
            }
            ...
            const regexMsg = regexError instanceof Error
              ...
              : "Unknown error";
            console.error(
              `[${functionVersion}] Regex extraction also failed${
                userId ? ` (User: ${userId})` : ""
              }: ${regexMsg}`,
            );
            console.error(
              `[${functionVersion}] Complete raw response${
                userId ? ` (User: ${userId})` : ""
              }: ${aiResponseContent}`,
            );
          }
      ...
      if (!parsedSuccessfully) {
        console.warn(
          `[${functionVersion}] Using fallback for continuation: Default title, full raw response as content (if available).`,
        );
      ...
      if (!finalContent) { // If content is still empty after parsing/fallback and cleaning
        console.error(
          `[${functionVersion}] Critical error: Final continuation content is empty after all processing.`,
        );
        finalContent =
          "La historia no pudo continuar esta vez. Intenta con otra opción o una nueva dirección.";
        // Optionally throw, but providing a message might be better UX for continuations
      }

      console.log(
        `[${functionVersion}] Final continuation result. reqId=${requestId} title="${finalTitle}" contentLength=${finalContent.length}`,
      );
    ...
    console.log(
      `[${functionVersion}] Action ${action} completed successfully. reqId=${requestId} userId=${userId}.`,
    );
    ...
    console.error(
      `Error in ${functionVersion} (reqId: ${requestId}, User: ${
        userId || "UNKNOWN"
    ...
    else if (lowerMessage.includes("límite de continuaciones")) {
      statusCode = 403;
    } else if (
      lowerMessage.includes("json in body") ||
      lowerMessage.includes("inválido/ausente") ||
      lowerMessage.includes("requerido")
    ) statusCode = 400;
    else if (
      lowerMessage.includes("bloqueada por seguridad") ||
      lowerMessage.includes("respuesta ia vacía") ||
      lowerMessage.includes("filtro de contenido")
    ) statusCode = 502;
    ...
    return new Response(
      JSON.stringify({
        error:
          `Error procesando solicitud (${requestedAction}): ${error.message}`,
        requestId,
      }),
      {
        status: statusCode,
